...
export default function App() {
  ...
  const [zoomedApp, setZoomedApp] = useState<AppId | null>(null)
  ...
  const closeApp = useCallback((appId: AppId) => {
    ...
    setZoomedApp(prev => prev === appId ? null : prev)
  }, [])
  ...
  const isFullScreen = zoomedApp !== null
  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-black">
      ...
      {isFullScreen && (
        <div
          className="fixed top-0 left-0 right-0 h-2 z-[10001]"
          onMouseEnter={() => setMenuBarRevealed(true)}
        />
      )}
      <MenuBar
        ...
        autoHidden={isFullScreen}
        revealed={!isFullScreen || menuBarRevealed}
        onRevealChange={setMenuBarRevealed}
        onOpenApp={openApp}
      ...
      <div className="absolute inset-0 top-8">
        {(Object.entries(windowStates) as [AppId, WindowState][]).map(([appId, state]) => {
          ...
          const desktopMaxW = appId === 'experience' ? 1000 : 900
          const desktopW = Math.min(Math.round(window.innerWidth * 0.75), desktopMaxW)
          const availH = window.innerHeight - 32 /* menubar */ - 80 /* dock */
          const desktopH = appId === 'about'
            ? Math.min(app.height, availH)
            : Math.min(Math.round(availH * 0.85), 720)
          ...
          return (
            <Window
              ...
              isTablet={isTablet}
              zoomTopInset={zoomedApp === appId && menuBarRevealed ? 32 : 0}
              onClose={() => closeApp(appId)}
              onMinimize={() => minimizeApp(appId)}
              onFocus={() => bringToFront(appId)}
              onZoomChange={zoomed => {
                setZoomedApp(zoomed ? appId : null)
